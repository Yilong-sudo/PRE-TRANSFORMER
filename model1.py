...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, hidden_size, **config):
        ...
        self.input_dim_target = config['input_dim_drug']
        self.max_d = config['max_drug_seq']
        self.max_p = config['max_drug_seq']
    ...
    def forward(self, rels, d, p, d_mask, p_mask):
        ...
        t_scores = torch.softmax(t_scores, dim=1)

        h_final = h_scores.unsqueeze(-1) * d_encoded_layers * p_aug
        t_final = t_scores.unsqueeze(-1) * p_encoded_layers * d_aug

        h_final = h_final.sum(1)
        t_final = t_final.sum(1)
