...
def load_drug_mol_data(args):
    ...
    unique_smiles = list(drug_smile_dict.values())
    print(f"Processing {len(unique_smiles)} unique drugs...")
    
    # 选择处理方式
    
    print("Using optimized sequential processing...")
    drug_features_dict = load_drug_mol_data_optimized(unique_smiles)
    
    save_data(drug_features_dict, 'drug_data.pkl', args)
    return drug_features_dict


def load_drug_mol_data_optimized(smiles_list):
    """优化版本的药物分子数据加载函数"""
    from rdkit.Chem import rdMolDescriptors
    import time
    
    print("Step 1: Pre-computing molecular fingerprints...")
    ...
    for i, smiles in enumerate(smiles_list):
        if i % 100 == 0:
            print(f"  Processing fingerprints: {i}/{len(smiles_list)}")
        
        try:
            mol = Chem.MolFromSmiles(smiles)
            if mol is not None:
                # 使用新的API避免deprecation warning
                try:
                    # 尝试使用新的API
    ...
    print(f"Fingerprint computation completed in {time.time() - start_time:.2f} seconds")
    print(f"Valid molecules: {len(valid_smiles)}/{len(smiles_list)}")
    
    print("Step 2: Computing similarity matrix...")
    start_time = time.time()
    
    drug_features_dict = {}
    total_comparisons = len(valid_smiles) * (len(valid_smiles) - 1)
    completed = 0
    
    for i, smiles_i in enumerate(valid_smiles):
        if i % 50 == 0:
            progress = (completed / total_comparisons) * 100 if total_comparisons > 0 else 0
            print(f"  Similarity computation: {i}/{len(valid_smiles)} drugs ({progress:.1f}%)")
        
        fp_i = mol_fingerprints[smiles_i]
        sim_features = []
        
        for j, smiles_j in enumerate(valid_smiles):
            if smiles_i != smiles_j:
                fp_j = mol_fingerprints[smiles_j]
                sim = DataStructs.TanimotoSimilarity(fp_i, fp_j)  # 使用更标准的Tanimoto相似性
                sim_features.append(sim)
                completed += 1
        
        drug_features_dict[smiles_i] = np.array(sim_features)
